 ...
 *
 */
int imprimirDosArrayInt (int* pArray,int* pArray2, int size)
{
	int i;
	int ret =-1;
	if(pArray != NULL && pArray2 != NULL && size>0)
	{
		for(i=0 ; i<size ; i++)
		{
			printf("El indice [%d] tiene como edad %d y salario %d \n",i,pArray[i],pArray2[i]);
		}
		ret = 0;
	}
	return ret;
